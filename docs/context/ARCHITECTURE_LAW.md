...
## 18. Безопасность и конфиденциальность (Security & Privacy)

- **Принцип минимизации данных**: хранить и передавать только необходимые поля; PII маскируется/редактируется до передачи в LLM или логов.
- **Least privilege**: доступ к `Registry`, prompts и execution-инфраструктуре — по ролям, через централизованный ACL.
- **Шифрование**: данные at-rest и in-transit должны быть зашифрованы; ключи управляются через KMS.
- **Prompt redaction**: перед логированием или внешней передачей — автоматическая фильтрация секретов/PII.
- **Audit trail**: все доступы к секретам, изменение prompt‑версий, операции миграции — логируются и подлежат ревью.
- **Data retention**: политики хранения и удаления данных документируются в `docs/policies/data_retention.md`.

---

## 19. Тестирование и QA (Testing & QA)

- **Prompt tests**: canonical prompts имеют набор unit/integration тестов (expected response patterns, invariants).
- **Behavioral regression**: при обновлении prompt_version или модели — обязательный набор regression тестов на ключевые сценарии.
- **End‑to‑end**: тесты покрывают контрактные входы/выходы компонентов и инструментов; mocking LLM — через fixtures.
- **Canary / staged rollout**: новые prompt_version и агенты выкатывать по группам с метриками качества и human‑in‑loop проверкой.
- **Monitoring & alerts**: авто‑оповещения при регрессии качества, повышении ошибки планов, росте confidence‑дрейфа.

---

## 20. Этика и ответственное использование (Ethics & Responsible AI)

- **Risk classification**: для каждого агента/компонента указывать risk_level (low/medium/high) и требуемый HITL уровень.
- **Bias & fairness audits**: регулярные проверки и документирование результатов в `docs/reports/`.
- **Human override**: для high‑risk решений обязательный human approval до ACTION.
- **Transparency**: пользователю/оператору доступны justification + prompt_id/prompt_version для критических решений.
- **Opt-out & consent**: механизмы отказа от автоматизированных действий и правила работы с чувствительными данными.

---

## 21. Appendix: Change log и контакты

- **Change log**: все изменения `ARCHITECTURE_LAW.md` фиксировать в `docs/CHANGELOG.md` с указанием owner и rationale.
- **Contacts**: поле `owner` в каждом сервисном doc; основные контакты проекта перечислены в `README.md` и `docs/contacts.md`.
- **Ревью**: крупные архитектурные изменения — ревью владельцами affected services и владельцем `ARCHITECTURE_LAW.md`.

---

<!-- End of appended sections 18-21 -->