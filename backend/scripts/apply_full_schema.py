...
SQL_PATH = Path(__file__).resolve().parents[1] / "sql" / "full_initial_schema.sql"
...
def apply_with_sqlalchemy(sql: str):
    ...
    print("Applied full initial schema via SQLAlchemy.")
    return True
...
def main():
    ...
    if apply_with_sqlalchemy(sql):
        return 0
    print("Failed to apply full schema: ensure DATABASE_URL and SQLAlchemy available", file=sys.stderr)
    return 2
