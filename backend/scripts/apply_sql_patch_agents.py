...
SQL_PATH = Path(__file__).resolve().parents[1] / "sql" / "patch_agents_add_columns.sql"
...
def apply_with_sqlalchemy(sql: str):
    ...
    print("Applied agents patch via SQLAlchemy.")
    return True
...
def apply_with_psycopg2(sql: str):
    ...
    try:
        ...
        conn.close()
    print("Applied agents patch via psycopg2.")
    return True
