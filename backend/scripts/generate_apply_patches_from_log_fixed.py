...
Usage:
  Set env var DATABASE_URL and run:
    python backend/scripts/generate_apply_patches_from_log_fixed.py --log backend/logs/latest_test_run.txt
"""
...
def parse_log(path: Path):
    ...
    for m in LOG_REGEX_COL.finditer(text):
        cols.append((m.group("table"), m.group("col")))
...
def main():
    ...
    print(sql_text[:400])
    if sql_text.strip().startswith("-- Nothing to patch"):
        print("No patches generated; nothing to apply.")
        return 0
